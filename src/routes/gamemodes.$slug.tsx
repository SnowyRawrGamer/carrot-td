...
export const Route = createFileRoute("/gamemodes/$slug")({
  component: GamemodeDetail,
});
...
function GamemodeDetail() {
  ...
  const { data: mode, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("gamemodes" as any)
        .select("*, added_updates:update_gamemodes(update:updates(id, name, slug)), removed_update:updates!removed_update_id(id, name, slug)")
        .eq("slug", slug)
        .maybeSingle();
  ...
  if (!mode) return <Page><Card className="p-8 text-center">Gamemode not found. <Link to="/gamemodes" className="text-primary underline">Back to gamemodes</Link></Card></Page>;
  ...
  return (
    <Page>
      <Link to="/gamemodes" className="inline-flex items-center text-sm text-muted-foreground hover:text-foreground mb-4">
        <ArrowLeft className="h-4 w-4 mr-1" /> All gamemodes
      </Link>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
          <Card className="overflow-hidden p-0">
            ...
            <div className="p-6">
              <h1 className="text-3xl font-bold mb-4">{mode.name}</h1>
              {mode.description && (
